...
class cliprz_view
{
    ...
    {
        if(empty($_SESSION['cliprz_template']))
        {
            $name_template = cliprz::system('config')->get('template','name');
        }
        else
        {
            $name_template = $_SESSION['cliprz_template'];
        }

        $file = PROJECT_PATH.'templates/'.$name_template.'/'.cliprz::rds($filename,'both').'.view'.EXT;
