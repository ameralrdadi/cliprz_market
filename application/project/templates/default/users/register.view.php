  ...
  <form class="form-signin" action="<?=URL?>check_register" method="POST">
	<?php if(isset($error)){ ?><div class="alert alert-error"><?=$error?></div><? } ?>
    <h2 class="form-signin-heading"><?=lang('register').' '.lang('user')?></h2>
    <input type="text" class="input-block-level" placeholder="<?=lang('username')?>" name="username">
    ...
    <button class="btn btn-large btn-primary" type="submit"><?=lang('register')?></button>
